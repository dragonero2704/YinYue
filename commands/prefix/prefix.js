...
module.exports = {
    ...
    async run(msg, args, bot) {
        ...
        if(!prefix){
            if(!curPrefix) return msg.reply({embeds:[titleEmbed(msg.guild, `[ Prefix ] : [ - ]`)]})
            return msg.reply({embeds:[titleEmbed(msg.guild, `[ Prefix ] : [ ${curPrefix.prefix} ]`)]})
        }
    ...
    async execute(interaction, bot){
        ...
        let curPrefix = await Prefixes.findOne({ where: { serverId: id } })
        if(!prefix){
            if(!curPrefix) return interaction.reply({embeds:[titleEmbed(interaction.guild, `[ Prefix ] : [ - ]`)]})
            return interaction.reply({embeds:[titleEmbed(interaction.guild, `[ Prefix ] : [ ${curPrefix.prefix} ]`)]})
        }
        prefix = prefix.substring(0,1);
        if (!curPrefix) {
            console.log(`Changing prefix for ${id}:'${prefix}'`)
